...
for s in "${sizes[@]}"; do
    for d in "${dwidth[@]}"; do
        gcc -DSIZE="$s" -DDWIDTH="$d" -static -std=c99 -O3 -msse2 -I ../gem5/util/m5 m5op_x86.S rowop.S** "$filename".c -o "$filename".exe
        cd ../gem5/
        ...
        # ./build/X86/gem5.opt configs/example/se.py --cpu-type=detailed --caches --l2cache --mem-type=DDR4_2400_x64 --mem-size=8192MB -c /home/MIMDRAM/18872_SHIF-DRAM/microworkloads/"$filename".exe -o > ../microworkloads/temp_results/"$s"_"$d".txt
        cd ../microworkloads/
    done
done
